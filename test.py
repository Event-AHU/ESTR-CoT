...
def split_text(text):
    ...
    for char in text:
        if is_chinese_char(char):
            if buffer:
                tokens.extend(''.join(buffer).lower().split())  # 处理英文部分，并转换为小写
                buffer = []
            tokens.append(char)  # 直接添加中文字符
        else:
            buffer.append(char)  # 暂存英文字符
    
    if buffer:
        tokens.extend(''.join(buffer).lower().split())  # 最后处理剩余的英文部分，转换为小写
...
def eval_batch(batch, model):
    ...
    outputs = model.generate({"image": images, "prompt": questions, "image_path": image_path},max_length=80)
...
def main(args):
    ...
    elif args.model_name == "bliva_vicuna_lora":  # 确保这里匹配
        model, vis_processors, text_processors = load_model_and_preprocess(name=args.model_name, model_type="vicuna7b", is_eval=True, device=args.device)
        model = reload_best_model(model)    
    ...
    with open(output_file_path, "w") as f:
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            for image_path, label, prediction in zip(batch["image_path"], batch["text_output"], metrics["answers"]):
